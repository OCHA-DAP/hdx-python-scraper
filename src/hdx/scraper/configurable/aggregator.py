...
class Aggregator(BaseScraper):
    ...
    def get_regional_scrapers(
        cls, region_config, hrp_countries, iso3_to_region_and_hrp, level_name, runner
    ):
        ...
        if not national_results:
            return cls.aggregation_scrapers
        ...
        for header, process_info in process_cols.items():
            name = f"{slugify(header.lower(), separator='_')}_{level_name}"
            ...
            if input_headers:
                ...
                for i, input_header in enumerate(input_headers):
                    try:
                        ...
                        logger.error(
                            f"{level_name} header {header} not found in national headers!"
                        )
                        exists = False
                        break
                if not exists:
                ...
                scraper = Aggregator(name, process_info, {level_name: headers})
                cls.aggregation_scrapers.append(scraper)
            else:
                try:
                    ...
                    scraper = Aggregator(
                        name, process_info, {level_name: headers}
                    )
                    cls.aggregation_scrapers.append(scraper)
                except ValueError:
                    logger.error(
    ...
    def process(self, output_values):
        ...
        if action == "sum" or action == "mean":
            ...
            for region, valuelist in output_values.items():
                toeval = formula
                for regional_scraper in self.aggregation_scrapers:
    ...
    def run(self):
        level_name = next(iter(self.headers.keys()))
        output_valdicts = self.get_values(level_name)
