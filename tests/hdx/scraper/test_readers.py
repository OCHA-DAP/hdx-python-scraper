...
class TestReaders:
    def test_read(self, configuration):
        ...
        with Download(user_agent='test') as downloader:
            datasetinfo = {'dataset': 'sahel-humanitarian-needs-overview', 'format': 'csv'}
            headers, iterator = read(downloader, 'test', datasetinfo, a='b')
            ...
            assert datasetinfo == {'dataset': 'sahel-humanitarian-needs-overview', 'format': 'csv', 'headers': 1, 'date': '2016-09-01',
            ...
            with pytest.raises(ValueError):
                datasetinfo = {'format': 'unknown'}
                read(downloader, 'test', datasetinfo)
            with pytest.raises(ValueError):
                datasetinfo = {'dataset': 'sahel-humanitarian-needs-overview', 'format': 'json'}
                read(downloader, 'test', datasetinfo)